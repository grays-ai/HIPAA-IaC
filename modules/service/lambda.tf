...
resource "aws_iam_policy" "lambda_combined_permissions" {
  name        = join("-", [var.name, "lambda-combined-permissions", var.stage, var.deploy_id])
  description = "Allow Lambda full S3 access and manage ENIs in a VPC"
...
resource "aws_iam_role_policy_attachment" "lambda_combined_permissions_attach" {
  role       = aws_iam_role.lambda.name
  policy_arn = aws_iam_policy.lambda_combined_permissions.arn
}
...
resource "aws_lambda_function" "data_copy_lambda" {
  ...
  handler       = var.lambda_handler # This should match what's in your lambda code. E.g., "index.handler" if your file is `index.js` and you export a function called `handler`.
